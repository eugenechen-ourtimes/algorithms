...
void runShortestPathAlgo(const char *graphFile, int sourceVertex) {
    WeightedGraph graph;
    int weightedGraph__constructor_status = weightedGraph__constructor(&graph, graphFile);
    if (weightedGraph__constructor_status != 0) {
    ...
    sourceVertex--;
    int findShortestPathStatus = findShortestPathWeightedGraph(&graph, sourceVertex, distances, predecessors);
    if (findShortestPathStatus != 0) {
        printf("error finding shortest path\n");
        weightedGraph__destructor(&graph);
        free(distances);
        free(predecessors);
        return;
    }
